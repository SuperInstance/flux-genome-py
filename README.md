...
One fixed genome. Adaptive protein expression. The same rigid Tensor-Penrose DNA structure serves every domain through regulated gene expression.
...
| Ribosome | Reads DNA, assembles constraint proteins |
| Proteins | Executable constraint-checking procedures |
| Gene Expression | Which proteins get built depends on environmental context |
| Promoters/Enhancers | Genes that activate other genes |
...
**The key insight:** The genome is FIXED. Expression is ADAPTIVE. Different environments (maritime, medical, automotive, aerospace, industrial) produce different constraint procedures from the same underlying genome.
...
# Get the built-in constraint genome (25 genes, 5 domains)
...
The complete Tensor-Penrose DNA — fixed, contains all possible proteins:
...
Reads the genome and assembles proteins:
...
The full expression pipeline — genome + environment → proteins → results:
...
The `builtins` module provides 20 constraint checker factories:

- `make_range_check(lo, hi)` — bounds checking
...
- `make_noise_floor_check(floor)` — minimum signal level
...
`constraint_genome()` returns a genome with 25 genes across 5 domains:
